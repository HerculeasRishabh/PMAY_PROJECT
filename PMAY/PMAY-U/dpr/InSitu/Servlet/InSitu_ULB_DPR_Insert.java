...
import dpr.InSitu.connection.*;

/**
 * Servlet implementation class InSitu_ULB_DPR_Insert
 */
@WebServlet("/InSitu_ULB_DPR_Insert")
public class InSitu_ULB_DPR_Insert extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InSitu_ULB_DPR_Insert() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    byte SEWERAGE = Byte.parseByte (request.getParameter("sewerage"));
	    byte ROAD = Byte.parseByte (request.getParameter("road"));
	    byte WATER_DRAIN = Byte.parseByte (request.getParameter("waterDrain"));
	    byte ELECTRIFICATION = Byte.parseByte (request.getParameter("electrification"));
	    byte WASTE_MANAGEMENT = Byte.parseByte (request.getParameter("wasteManagement"));
	    String OTHER_INFRA = request.getParameter("anyOther");
	    byte DESASTER_RESISTANT = Byte.parseByte (request.getParameter("disaster"));
	    ...
	    byte GREEN_TECH = Byte.parseByte (request.getParameter("technologyAdopted"));
        ...
        obj.inSitu_DPR_Insert(STATE, DISTRICT, CITY, SLUM, PORJECT_NM, SLNA_NM, IA_NAME, PROJECT_COST, PROJECT_DURATION, SLUM_STATUS,
                SLUM_AREA, AREA_REHAB, SLUM_POPULATION, GEN_OLD, SC_OLD, ST_OLD, OBC_OLD, MINORITY_OLD, GEN_NEW, SC_NEW, ST_NEW, OBC_NEW, 
                MINORITY_NEW, HOUSE_PROPOSED, TOTAL_CAPR_AREA, PMAY_GUIDELINES, OPEN_PPP, EXISTING_FSI, FSI_PROVIDED, OTHER_INCENTIVE,
                CENTRAL_GRANT, STATE_GRANT, ULB_GRANT, BEN_SHARE, TECH_SPECIFICATION, TEMP_ARRANGEMENT, TRUNK_INFRA, WATER, SEWERAGE, 
                ROAD, WATER_DRAIN, ELECTRIFICATION, WASTE_MANAGEMENT, OTHER_INFRA, DESASTER_RESISTANT, QUALITY_ASSURANCE, QNM, ENCUMB_FREE, 
                GREEN_TECH, SLAC_COMMENT, OTHER_INFO, DGTL_SIGN_ULB);
